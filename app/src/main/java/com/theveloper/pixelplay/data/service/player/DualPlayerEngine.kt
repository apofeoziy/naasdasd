...
class DualPlayerEngine @Inject constructor(
    ...
    private val mrbifyStreamProxy: com.theveloper.pixelplay.data.stream.MrbifyStreamProxy,
            ...
            try {
                ...
                if (currentIndex != C.INDEX_UNSET) {
                    ...
                    if (currentIndex + 1 < playerA.mediaItemCount) {
                        ...
                        } else if (nextUri?.scheme == "netease" || nextUri?.scheme == "qqmusic" || nextUri?.scheme == "mrbify") {
                    ...
                    if (currentIndex - 1 >= 0) {
                        ...
                        } else if (prevUri?.scheme == "netease" || prevUri?.scheme == "qqmusic" || prevUri?.scheme == "mrbify") {
    ...
    private fun buildPlayer(handleAudioFocus: Boolean): ExoPlayer {
        ...
        val resolver = object : ResolvingDataSource.Resolver {
            override fun resolveDataSpec(dataSpec: DataSpec): DataSpec {
                ...
                if (scheme == "telegram" || scheme == "netease" || scheme == "qqmusic" || scheme == "mrbify") {
    ...
    suspend fun resolveCloudUri(uri: Uri): Uri {
        ...
        val resolved: Uri? = when (uri.scheme) {
            ...
            "mrbify" -> resolveMrbifyUriAsync(uriString)
            else -> null
        }
    ...
    private suspend fun resolveMrbifyUriAsync(uriString: String): Uri? {
        Timber.tag("DualPlayerEngine").d("Async resolving Mrbify URI: $uriString")

        if (!mrbifyStreamProxy.isReady()) {
            Timber.tag("DualPlayerEngine").w("MrbifyStreamProxy not ready, awaiting...")
            val proxyReady = mrbifyStreamProxy.awaitReady(5_000L)
            if (!proxyReady) {
                Timber.tag("DualPlayerEngine").e("MrbifyStreamProxy not ready after timeout")
                return null
            }
        }
        ...
        Timber.tag("DualPlayerEngine").w("Failed to resolve Mrbify URI: %s", uriString)
        return null
    }
    ...
    suspend fun resolveMediaItem(mediaItem: MediaItem): MediaItem {
        ...
        if (scheme != "telegram" && scheme != "netease" && scheme != "qqmusic" && scheme != "mrbify") return mediaItem
